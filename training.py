...
class MXNetTrainingWorker(object):
    ...
    def prepare_train_config(self):
        ...
        epoch_end_cb = [
            ...
            self.train_ins.get_epoch_end_callback(
                "mxnet", epoch_interval=snapshot_interval_epochs, other_files=[])
        ]
    ...
    def prepare_model(self):
        ...
        # 用户可以直接使用训练框架来读取模型
        origin_model_path = self.train_ins.get_model_base_path() + "/train.symbol.json"
        weight_file_path = self.train_ins.get_model_base_path() + "/weight.params"
